    ...
    <div class="row justify-content-center">
        <div class="col-lg-8 card-wrapper ct-example">
            <div class="card">
                ...
                <form method="POST" action="{{ route('admin.trends.update', $trend->_id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                    ...
                    <div class="card-body">
                        ...
                        <select name="field_id" class="form-control form-select-lg mb-3" aria-label=".form-select-lg example">
                            @foreach($fields as $field)
                                <option {{ $field->_id == $trend->field_id ?? "selected" }} value="{{ $field->_id }}">{{ $field->name }}</option>
                            @endforeach
                        </select>
                        @error('field_id')
                        <span class="message text-danger">{{ $message }}</span>
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <input type="text" name="title" value="{{ $trend->title }}" class="form-control" id="exampleFormControlInput1">
                            @error('title')
                                <span class="message text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <textarea name="content" id="editor1" rows="10" cols="80">{{ $trend->content }}</textarea>
                            @error('content')
                                <span class="message text-danger">{{ $message }}</span>
                            @enderror
                        </div>
