...
jQuery(function($) {
    ...
    var UI = {
        ...
        /*scrollHori: function(element) {
            ...
            var wrapScroll= $('.wrap-scroll').outerWidth();

            var itemsLength = $('.list-scroll .item').length;
            var itemSize = $('.list-scroll .item').outerWidth(true);
            var getTotalSize = function() {
                return itemsLength * itemSize;
            };
            var menuSize = getTotalSize();
            console.log(menuSize);
            var getScrollPosition = function() {
                return $('.list-scroll').scrollLeft();
            };

            $('.list-scroll').on('scroll', function() {
                var scrollPosition = getScrollPosition();
                var menuSize = getTotalSize();
                if (scrollPosition == 0) {
                    $('.arrow-scroll.left').addClass('hide');
                }else{
                    $('.arrow-scroll.left').removeClass('hide');
                }
                
                if (scrollPosition == menuSize - wrapScroll) {
                    $('.arrow-scroll.right').addClass('hide');
                }else{
                    $('.arrow-scroll.right').removeClass('hide');
                }

            });
            $('.arrow-scroll.right').click(function () {
                let position = $('.list-scroll').scrollLeft();
                $('.list-scroll').animate({scrollLeft: position + wrapScroll}, 300);
            })
            $('.arrow-scroll.left').click(function () {
                let position = $('.list-scroll').scrollLeft();
                $('.list-scroll').animate({scrollLeft: position - wrapScroll}, 300);
            })
        },*/
